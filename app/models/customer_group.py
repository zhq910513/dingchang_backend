...
class CustomerGroup(Base):
    ...
    team_name = Column(String(32), nullable=True, comment="团队名称（历史归属/标记，可空；参与唯一键）")
    ...
    created_by = Column(Integer, ForeignKey("user_new.id"), nullable=True, comment="创建人用户ID（FK -> user.id，可空）")
    ...
    __table_args__ = (
        ...
        # 生产 DDL 使用 KEY `created_by`（非 ix_* 命名）
