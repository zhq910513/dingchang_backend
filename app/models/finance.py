...
class FinanceRecord(Base):
    ...
    order_id = Column(Integer, ForeignKey("order_new.id"), nullable=False, comment="订单ID（FK -> order.id，一对一）")
    supplier_id = Column(Integer, nullable=True, comment="供应商ID（可空；当前无外键）")

    upstream_paid = Column(Integer, nullable=False, server_default=text("'0'"), comment="上游是否已支付（0/1，int）")
    downstream_paid = Column(Integer, nullable=False, server_default=text("'0'"), comment="下游是否已支付（0/1，int）")
