...
class Order(Base):
    ...
    created_by = Column(Integer, ForeignKey("user_new.id"), nullable=False,
                        comment="创建人用户ID（FK -> user.id）")
    salesperson_id = Column(Integer, ForeignKey("user_new.id"), nullable=False,
                            comment="业务员用户ID（FK -> user.id）")

    customer_group_id = Column(Integer, ForeignKey("customer_group_new.id"), nullable=True,
                               comment="客户组ID（FK -> customer_group.id，可空）")
    channel_group_id = Column(Integer, ForeignKey("channel_group_new.id"), nullable=True,
                              comment="渠道组ID（FK -> channel_group.id，可空）")

    is_finished = Column(Boolean, nullable=False, server_default=text("0"), comment="是否完成（0/1）")
    is_rebate = Column(Boolean, nullable=False, server_default=text("0"), comment="是否返利（0/1）")
    is_paid = Column(Boolean, nullable=False, server_default=text("0"), comment="是否已支付（0/1）")

    status = Column(Integer, nullable=False, server_default=text("0"), comment="订单状态（int 枚举）")
    audit_status = Column(Integer, nullable=False, server_default=text("0"), comment="审核状态（int 枚举）")

    dynamic_data = Column(JSON, nullable=False, comment="订单级动态数据（JSON，后续建议收口为固定字段集）")
    ocr_raw_json = Column(JSON, nullable=False, comment="OCR原始返回快照（JSON，按 slot_key 存 raw/error）")
    ...
    # 关系：字典表
    customer_group = relationship("CustomerGroup", back_populates="orders", lazy="selectin", doc="客户组")
...
class OrderImage(Base):
    ...
    slot_key = Column(String(64), nullable=False, comment="卡槽Key（slot_key，与 slot_field_config 对齐）")
    ...
    image_file_id = Column(Integer, ForeignKey("image_file_new.id", ondelete="SET NULL"), nullable=True,
    ...
    # 关系
    order = relationship("Order", back_populates="images", lazy="selectin", doc="所属订单")
    image_file = relationship("ImageFile", back_populates="order_images", lazy="selectin", doc="关联图片文件（可空）")
