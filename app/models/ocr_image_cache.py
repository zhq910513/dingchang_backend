...
class OcrImageCache(Base):
    """
    OCR 图片缓存表（以 storage_key 为粒度，不依赖 image_file_id）

    说明：
    - 同一 storage_key + provider + api_type + side 唯一
    - result 存 OCR 返回 JSON（通常为原始返回）
    - 用于避免重复调用 OCR（节省成本与延迟）
    """
    ...
    result = Column(
        JSON,
        nullable=False,
        comment="OCR结果JSON（通常为原始返回，用于复用）",
    )
