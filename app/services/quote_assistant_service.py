...
def _sanitize_platform_dialog_message(
    ...
    *,
    ...
    if not text:
        return default_message
    text = re.sub(r"^[A-Z]{2,}[A-Z0-9_]*(?:[-_]\d+)?\s*[-_：:，,;；]?", "", text).strip()
    text = sanitize_quote_user_message(
        text,
        default_message or text,
    ...
    *,
    ...
    candidates.extend(
        [
            ...
            _json_obj(platform_response.get("response")).get("businessControlMsg"),
            _json_obj(platform_response.get("response")).get("businessMsg"),
            _json_obj(platform_response.get("response")).get("errorMessage"),
            _json_obj(platform_response.get("response")).get("statusText"),
...
def _slot_extracted_data_map(images_by_slot: Dict[str, List[Dict[str, Any]]]) -> Dict[str, Dict[str, Any]]:
    ...
    for slot_key, rows in (images_by_slot or {}).items():
        ...
        for image in rows or []:
            merged = _merge_data(merged, _slot_filtered_extracted_fields(slot_key, image))
        if merged:
            out[slot_key] = merged
    return out
...
def _active_image_extracted_data(images_by_slot: Dict[str, List[Dict[str, Any]]]) -> Dict[str, Any]:
    ...
    for slot_key in ("driving_license_main", "driving_license_sub", "vehicle_cert", "idcard_back", "idcard_front", "related"):
        merged = _merge_data(merged, slot_data.get(slot_key, {}))
    return _clean_quote_dynamic_data(merged, derive_owner_name=True)
...
def _normalize_quote_product_exclusions(value: Any) -> List[str]:
    ...
    for item in items:
        ...
        if label == QUOTE_LOSS_LABEL and label not in exclusions:
...
def _detect_professional_quote_command(text: Any) -> Dict[str, Any]:
    ...
    if not compact:
        return {}
    if compact in PICC_JIAOSAN_COMMANDS:
        return {
            "is_quote": True,
            "entities": {
                "platform_code": "PICC",
    ...
    if compact in PICC_FULL_COVER_COMMANDS:
        return {
            "is_quote": True,
            "entities": {
                "platform_code": "PICC",
...
def extract_quote_config_overrides(text: Any) -> Dict[str, Any]:
    ...
    connector = r"(?:保额|金额|额度|限额|改成|改为|调整为|调整到|调成|改到|设置为|设为|变成|调整|改|到|为)?"
    separator = r"(?:[:：=+＋])?"
    number = r"(-?\d+(?:[,，]\d{3})*(?:\.\d+)?)"
...
async def get_pending_duplicate_quote_confirm_payload(
    ...
    *,
    ...
    platform_code = case.platform_code or task.platform_code or ""
    platform_name = case.platform_name or task.platform_name or platform_code
    dialog = _duplicate_quote_platform_dialog(
        warning=warning,
        platform_code=platform_code,
        platform_name=platform_name,
        trace_id=trace_id,
...
async def _stop_quote_for_material_change(
    ...
    *,
    ...
    return (
        f"{platform_label}报价材料已更新，我已停止本次报价；请确认材料后重新发起报价。",
        {
            "status": "success",
            "intent": "quote",
            "trace_id": trace_id,
            "data": _mk_data(
                result_status=RESULT_NOT_READY,
                message=QUOTE_MATERIAL_CHANGED_MESSAGE,
...
def _extract_joint_sales_image_adjustment(text: Any) -> Dict[str, Any]:
    ...
    number = r"(-?\d+(?:[,，]\d{3})*(?:\.\d+)?)"
    pattern = rf"^(?:请)?(?:把|将)?(?:{alias_group})(?:保费|金额|保额)?(?:[:：=+＋]|改成|改为|调整为|调整到|调成|改到|设置为|设为|变成|调整|改|到|为)?{number}(万|元)?$"
...
async def _handle_joint_sales_image_adjustment_message(
    ...
    *,
    ...
    submitted_snapshot["joint_sales_image_adjustment"] = {
        ...
        "resolved_amount": _quote_money_text(joint_sales_amount),
        "raw_text": text,
    }
    request_payload = {
    ...
    task = QuoteTask(
        quote_case_id=case.id,
        platform_code=source_task.platform_code or case.platform_code or adjusted_result.get("platform_code") or "PICC",
        platform_name=platform_name or source_task.platform_code or case.platform_code,
    ...
    *,
    ...
    if not await _quote_snapshot_material_is_current(db, case=case, snapshot=snapshot):
        ...
        task = QuoteTask(
            ...
            result_payload={},
            submitted_snapshot=snapshot,
            error_detail=QUOTE_MATERIAL_CHANGED_MESSAGE,
            started_at=_now(),
            finished_at=_now(),
        )
        db.add(task)
    ...
    if not _is_runtime_quote_success(quote_status):
        ...
        if _is_runtime_session_expired_result(quote_runtime_result):
            ...
            draft_account_type = (
                _to_str(request_body_draft.get("accountTypeName")).strip()
                or _to_str(_json_obj(snapshot.get("platform_default_config")).get("resolved_type_name")).strip()
                or _to_str(platform_account.account_type_name).strip()
                or "当前账号类型"
            )
            reply_lines = [
...
async def handle_quote_message(
    ...
    *,
    ...
    if waiting_pair and quote_field_overrides:
        case, task = waiting_pair
        await _expire_waiting_sms_task(
            db,
            case=case,
    ...
    if (quote_field_overrides or short_quote_command or transfer_vehicle_text_data) and (not platform_code or not platform_name):
    ...
    if not platform_name and (quote_field_overrides or transfer_vehicle_text_data):
    ...
    if missing:
        ...
        lines = [
            f"{platform_name}报价资料还不完整，已中断本次报价。",
        ]
        if override_summary:
        ...
        if attached_images:
            ...
            if review_count:
                lines.append(f"{review_count} 张图片特征不够明确，已先放入相关图片，不会覆盖关键材料；如需修正，请补一句图片说明后重新拖入。")
    ...
    if missing_account:
        ...
        return "\n".join(lines), {
            ...
            "data": _mk_data(
                result_status=RESULT_NEED_MORE,
                message="同平台账号暂不可用" if account_pool_unavailable else "报价资料已齐，等待配置可用平台账号",
                entities={**merged_entities, "quote_case_id": case.id, "order_id": case.order_id},
                payload=payload,
            ),
            "actions": [
    ...
    platform_default_config = _json_obj(snapshot.get("platform_default_config"))
    if selected_account_type_name and _to_str(platform_default_config.get("matched")).strip() != "account_type":
        case.status = CASE_STATUS_READY
        case.updated_at = _now()
        await _add_event(
            db,
...
async def handle_quote_material_status(
    ...
    *,
    ...
    if normalized_data != _json_obj(case.normalized_data) or missing != _json_list(case.missing_requirements):
        ...
        case.draft_order_data = normalized_data
        case.missing_requirements = missing
        if case.status not in {CASE_STATUS_QUOTED, CASE_STATUS_WAITING_SMS}:
            case.status = CASE_STATUS_READY if not missing else CASE_STATUS_COLLECTING
        case.updated_at = _now()
        await db.flush()
    platform_account = await _select_quote_platform_account(
